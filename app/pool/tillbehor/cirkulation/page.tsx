...
import { cirkulation, poolkonstruktion } from "@/lib/pahlenProducts";

export const metadata: Metadata = { title: "Rening och cirkulation | Elite Pool & Spa" };

const sandfilterCount = cirkulation.filter(p => p.group === "Sandfilter" || p.group === "Filter tillbehör").length;
const pumparCount =
  cirkulation.filter(p => p.group === "Pumpar").length +
  poolkonstruktion.filter(p => p.group === "SBM stommar").length;

const subcategories = [
  {
    label: "Sandfilter",
    slug: "sandfilter",
    count: sandfilterCount,
    desc: "Purgo, Mundo, Altus och Tech Pro sandfilter från Pahlén — samt filtersand, AFM-media, Besgo-ventiler och FlowVis-mätare.",
    bg: "bg-amber-50",
    tag: "Pahlén",
  },
  {
    ...
    slug: "pumpar",
    count: pumparCount,
    desc: "Bronspump P2000 och P85, VISE-serien, BADU, SBM och agenturpumpar — med tillhörande styrboxar och tillbehör.",
    bg: "bg-blue-50",
    tag: "Pahlén",
  },
];

export default function CirkulationPage() {
  const total = sandfilterCount + pumparCount;
  ...
  return (
    <main className="min-h-screen bg-white">
          ...
          <div className="flex items-end justify-between gap-4 pb-10">
            <div>
              ...
              <h1 className="text-4xl font-black tracking-tight text-slate-900 sm:text-5xl lg:text-6xl">
                Rening och cirkulation
              </h1>
              <p className="mt-4 text-sm text-slate-500 leading-relaxed max-w-lg">
                Sandfilter, pumpar och tillbehör från Pahlén — allt för ett optimalt flödessystem i din pool.
              </p>
            </div>
      ...
      <section className="px-6 py-14 sm:px-12 lg:px-20">
        <div className="mx-auto max-w-7xl grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl">
          {subcategories.map(c => (
            <div key={c.slug} className="group flex flex-col rounded-2xl border border-slate-100 overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="flex flex-col flex-1 p-5">
                ...
                <div className="mt-5">
                  <Link
                    href={`/pool/tillbehor/cirkulation/${c.slug}`}
